...
trait SIATrait
{
    ...
    public function getSIALaporanExcel(array $classroomIds)
    {
        ...
        foreach ($attendances as $attendance) {
            ...
            if (!isset($report[$classType][$className])) {
                $report[$classType][$className] = [
                    'teacher_name' => $teacherName,
                    'students' => []
                ];
            }

            if (!isset($report[$classType][$className]['students'][$studentId])) {
                $report[$classType][$className]['students'][$studentId] = [
                    'name' => $attendance->student->name,
                    'gender' => $attendance->student->gender,
                    'months' => array_fill_keys($months, ['sick' => 0, 'permission' => 0, 'alpha' => 0, 'points' => 0]),
                    'total_tatib_points' => 0,
            ...
            if (in_array($attendance->status, ['sick', 'permission', 'alpha'])) {
                ...
                $report[$classType][$className]['students'][$studentId]['months'][$month][$attendance->status]++;
                $report[$classType][$className]['students'][$studentId]['months'][$month]['points'] += $tatibPoints;
            }
        }

        foreach ($report as &$classTypes) {
            foreach ($classTypes as &$classData) {
